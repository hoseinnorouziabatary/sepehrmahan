<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        try {
            $conn = new PDO("mysql:host=localhost;dbname=sepehrmahan" , "root" , "");
            $conn->setAttribute(PDO::ATTR_ERRMODE , PDO::ERRMODE_EXCEPTION);

            $statm = $conn->prepare("select * from users where username = :username");
            $statm->execute(compact('username'));

            $user = $statm->fetch(PDO::FETCH_OBJ);
            if($user) {
                if($user->password == $password) {
                    header("Location: http://mvcproject.test/auth/welcome.php");
                } else {
                    $errors['username'] = 'اطلاعات شما با هم مطابقت ندارد';
                }
            } else {
                $errors['username'] = 'چنین کاربری وجود ندارد';
            }
